...
export const ColorPicker = (props) => {
  const { setCurrentPickedColor, setSelectedElement } =
    useSettingsStorageDispatch();
  ...
  onMount(() => {
    colorpicker = Pickr.create({
      ...
      position:
        selectedMenuName() === "Speedo" ? "left-middle" : "right-middle",
  ...
  return (
    <div>
      <button
        id={`${props?.name}-colorpicker`}
        class={`${
          props?.isActive ? "bg-red-400" : ""
        } p-2 bg-[#0087D0] rounded-md hover:scale-125 ease-out duration-300`}
        onClick={handleClick}
      >
        <ColorPaletteIcon />
      </button>
