...
const RangeSlider = (props) => {
  ...
  return (
    <div class="flex items-center justify-center gap-4 relative pt-1">
      <label for="rangeWidth" class="form-label uppercase">
        {props?.name}
      </label>
      <input
        type="range"
...
export const Status = (props) => {
  const [circleProgressColors, setCircleProgressColors] = createStore(
    progressDefaultCircles
  );
  const settingsStorageState = useSettingsStorageState();
  const { getCurrentColor, setCurrentCircleWidth } =
    useSettingsStorageDispatch();
  ...
  return (
    <>
      <div class="p-2 flex flex-col gap-6">
        <For each={circleProgressColors}>
          {(progress, i) => (
            <div class="flex justify-evenly items-center">
              <h2 class="w-[100px] uppercase">
                {translate(progress.name, "Status")}
              </h2>
              <div style={"transform: scale(1.5);padding-right: 30px"}>
                <CircleProgress
                  ...
                  color={getCurrentColor(progress.name, "statusColors")}
                />
              </div>
              <ColorPicker
        ...
        <RangeSlider
          currentCircleWidth={currentWidth()}
          setWidth={setCurrentCircleWidth}
          name={translate("circleWidth", "Status")}
        />
      </div>
    </>
  );
