...
import { defaultMenuButtons } from "../../../../DefaultDatas";
...
const MainPage = (props) => {
  ...
  return (
    <div class="flex gap-4 justify-center border-b-2 border-[#0087D0] border-solid p-4">
      <For each={defaultMenuButtons}>
        {(menu, i) => (
          <A
            className={`p-2 ${
              currentPath() === menu.path ? "bg-blue-800" : "bg-sky-400"
            } uppercase rounded-md bg-[#0087D0] hover:scale-125 ease-out duration-300`}
            href={`/${menu.path}`}
            onClick={() => props?.onMenuClick(menu)}
          >
            {translate(menu.name)}
          </A>
        )}
...
const ExitIcon = (props) => (
  ...
  >
    <path
      class="fill-gray-800 hover:fill-red-700 hover:ease-in duration-100"
  ...
  return (
    <>
      <div class="relative flex justify-evenly items-center mt-5 z-10">
        <button
          class="p-2 bg-red-700 rounded-md hover:scale-125 ease-out duration-300"
          onClick={clickReset}
        >
        ...
        <div class="uppercase text-3xl">
          {title().length > 0 ? translate(title()) : ""}
        </div>
        <button
          class="p-2 bg-green-700 rounded-md hover:scale-125 ease-out duration-300"
          onClick={click}
        >
          <div class="flex gap-2 items-center justify-center">
            <SaveIcon />
...
export const SettingsPanel = () => {
  ...
  const currentSelectedElementName = () =>
    settingsStorageState.selectedElementName;
  ...
  return (
    <>
      <div
        class={`relative bg-[#17191A] w-[600px] h-[650px] rounded-[25px] ease-in duration-500 z-30 ${
          showPanel() ? "slideDown" : "slideUp"
        }`}
      >
        ...
        <SaveResetButtons
          ...
          showEdit={showEdit()}
          currentSelectedMenu={selectedMenuName()}
          selectedName={currentSelectedElementName()}
          onSaveAndResetButtonClick={handleClickSaveAndReset}
        />
        <Routes>
